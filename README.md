...
| `Esc` `Esc` | 老板键：连续按两下，藏掉版面内容只留空白网格（再按两下恢复）；设置面板 / 图片弹层开着时，Esc 先关它们 |
| `Alt+反引号` | 老板键：切回虎扑原页面，再按一次切回来（不写存储） |
...
- **老板键（`Esc` `Esc`）**：连按两下把「版面内容」全藏掉 —— 单元格数据、工作表标签名、编辑栏面包屑、分页计数、账号区、搜索框里的「虎扑」字样都不用，只留 Excel 外壳 + 一张空白网格（行列标和空白格照旧）；再按两下恢复。状态记在 `sessionStorage`，中途切去原生页面再回来也还是空白网格。
