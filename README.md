...
**主页面** —— 版面列表页（`/bxj` 这类）：帖子流 + 底部工作表标签（版面名 / 我的关注 / 热门专区 / 话题广场 / 版主）
...
| `Esc` `Esc` | 老板键：连续按两下，藏掉版面内容只留空白网格（单按一下 Esc 恢复）；设置面板 / 图片弹层开着时，Esc 先关它们，在原生页面时 Esc 切回 Excel |
| `Alt+反引号` | 老板键：切回虎扑原页面，再按一次（或按 Esc）切回来（不写存储） |
...
标题栏右侧有一对按钮：**📝 发帖**、**💬 回复**（带文字标签、跟着皮肤上色）—— 所有皮肤里都在，详见下面的「发帖 / 回帖」。

> 编辑栏**一直**显示这串面包屑和标题，点击帖子里的回复不会把它换成单元格内容 —— 所以「回上一层」的入口和当前页标题在任何时候都在，单元格内容看格子本身就行。

## 发帖 / 回帖

入口在**标题栏右侧**：「📝 发帖」「💬 回复」两个按钮（带文字标签、跟着皮肤上色）。放标题栏是因为腾讯文档皮肤不显示功能区、飞书皮肤的工具栏太长时也未必放得下，只有标题栏是所有皮肤都在的地方：
...
标题栏右上角的 ⚙ 按钮会弹出「Excel 选项」（长什么样见上面的[截图](#截图)），左边是分组导航，右边是全部可配置项，改完立即生效并存进油猴存储。

> **「Excel 模式」的开关不在这个面板里**，而在油猴菜单：点油猴图标 →「虎扑 Excel · 摸鱼模式」→「开关 Excel 模式」（列表里还会显示现在是开还是关）。因为它一关整个界面（连同齿轮）都会消失，放在页面内的面板里太容易误触、也太容易让人以为脚本坏了。菜单里另外还有一条「恢复默认设置并开启」兜底。

面板里的配置项：

| 分组 | 配置项 | 默认 |
| --- | --- | --- |
| 常规 | 皮肤：Office / 腾讯文档 / WPS / 飞书云文档 | Office |
| 常规 | 工作簿名称（浏览器标签标题） | 工作簿1 |
| 视图 | 显示账号区 / 发帖·回复（登录/消息/分享/批注 + 昵称头像 + 那两个按钮） | 开 |
| 视图 | 显示「路径」列 | 关 |
| 视图 | 冻结列标行 | 开 |
...
> 关掉「Excel 模式」（油猴菜单或 `Alt+E`）后界面会完全消失（也不再有齿轮），这时页面底部会弹一条 6 秒的提示告诉你怎么打开。再打开就点油猴菜单里那条「开关 Excel 模式」或按 `Alt+E`；**如果设置被改乱了、甚至整个界面不见了，油猴菜单「恢复默认设置并开启」或 `Alt+Shift+E` 兜底**，也可以在油猴的脚本编辑器 → 存储里删掉 `hx.cfg`。
...
- **老板键（`Esc` `Esc`）**：连按两下把「版面内容」全藏掉 —— 单元格数据、工作表标签名、编辑栏面包屑、分页计数、账号区、标题栏那对「发帖 / 回复」按钮、搜索框里的「虎扑」字样都不用，只留 Excel 外壳 + 一张空白网格（行列标和空白格照旧）；再按两下恢复。状态记在 `sessionStorage`，中途切去原生页面再回来也还是空白网格。
...
- 自己画整窗 Excel：标题栏 / 选项卡 / 功能区 / 编辑栏 / 行列标（A…Z、AA…）/ 冻结窗格 / 工作表标签 / 状态栏 / 缩放条，4 套皮肤（Office / 腾讯文档 / WPS / **飞书云文档**）；
